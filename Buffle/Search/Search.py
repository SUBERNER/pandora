    ...
    if deep_search:  # if enabled, will also go through all subfolders inside source
        ...
        for folder in folders:  # goes through each folder
            if type_search == "all":
                ...
                files.extend([f for f in os.scandir(folder) if f.is_file() and ((any(contain in f.name for contain in contains) and not inverse_search) or (any(contain not in f.name for contain in contains) and inverse_search))])  # gets all files with the substring inside source directory
    ...
    for entry in original_files:
        if entry.is_file():
            with open(entry, 'r') as file:
                text = file.read()
                if type_search == "all":
                    if ((all(contain in text for contain in contains) and not inverse_search) or (all(contain not in text for contain in contains) and inverse_search)):  # determines if contains is inside the file text
                        formatted_files.append(entry)
                elif type_search == "any":
                    if ((any(contain in text for contain in contains) and not inverse_search) or (any(contain not in text for contain in contains) and inverse_search)):  # determines if contains is not inside the file text
                        formatted_files.append(entry)
